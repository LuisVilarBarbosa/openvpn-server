...
if os.geteuid() != 0:
    ...
    functions.execute_command(["sudo", "python3", my_file_path])
    quit()
...
functions.execute_command(["apt-get", "install", "-y", "openvpn"])
functions.execute_command(["wget", "--no-clobber", "-P", installation_dir + "/", "https://github.com/OpenVPN/easy-rsa/releases/download/v3.0.4/EasyRSA-3.0.4.tgz"])
...
functions.replace_text_in_file("#set_var EASYRSA_REQ_COUNTRY", "set_var EASYRSA_REQ_COUNTRY", vars_path)
functions.replace_text_in_file("#set_var EASYRSA_REQ_PROVINCE", "set_var EASYRSA_REQ_PROVINCE", vars_path)
functions.replace_text_in_file("#set_var EASYRSA_REQ_CITY", "set_var EASYRSA_REQ_CITY", vars_path)
functions.replace_text_in_file("#set_var EASYRSA_REQ_ORG", "set_var EASYRSA_REQ_ORG", vars_path)
functions.replace_text_in_file("#set_var EASYRSA_REQ_EMAIL", "set_var EASYRSA_REQ_EMAIL", vars_path)
functions.replace_text_in_file("#set_var EASYRSA_REQ_OU", "set_var EASYRSA_REQ_OU", vars_path)
...
functions.execute_command(["nano", vars_path])
functions.execute_and_send_input_to_command(["./easyrsa", "init-pki"], "yes\n")
functions.execute_and_send_input_to_command(["./easyrsa", "build-ca", "nopass"], "\n")

# Step 3 — Creating the Server Certificate, Key, and Encryption Files
os.chdir(easyrsa_dir)
functions.execute_and_send_input_to_command(["./easyrsa", "gen-req",  configuration_variables.SERVER_NAME, "nopass"], "\n")
shutil.copy2(easyrsa_dir + "/pki/private/" + configuration_variables.SERVER_NAME + ".key", "/etc/openvpn/")
functions.execute_and_send_input_to_command(["./easyrsa", "sign-req", "server", configuration_variables.SERVER_NAME], "yes\n")
...
functions.execute_command(["./easyrsa", "gen-dh"])
functions.execute_command(["openvpn", "--genkey", "--secret", "ta.key"])
...
functions.makedirs(client_keys_dir)
functions.chmod_recursive(client_configs_dir, 0o700)
os.chdir(easyrsa_dir)
...
functions.replace_text_in_file(";group nogroup", "group nogroup", server_conf_path)

## (Optional) Push DNS Changes to Redirect All Traffic Through the VPN
if configuration_variables.BYPASS_DNS == "yes":
    functions.replace_text_in_file(";push \"redirect-gateway def1 bypass-dhcp\"", "push \"redirect-gateway def1 bypass-dhcp bypass-dns\"", server_conf_path)
elif configuration_variables.BYPASS_DNS == "no":
    functions.replace_text_in_file(";push \"redirect-gateway def1 bypass-dhcp\"", "push \"redirect-gateway def1 bypass-dhcp\"", server_conf_path)
...
functions.replace_text_in_file(";push \"dhcp-option DNS 208.67.222.222\"\n", "", server_conf_path)
functions.replace_text_in_file(";push \"dhcp-option DNS 208.67.220.220\"\n", dhcp_option_dns_full, server_conf_path)

## (Optional) Adjust the Port and Protocol
functions.replace_text_in_file("port 1194", "port " + configuration_variables.SERVER_PORT, server_conf_path)
functions.replace_text_in_file(";proto tcp\n", "", server_conf_path)
functions.replace_text_in_file("proto udp", "proto " + configuration_variables.SERVER_PROTOCOL, server_conf_path)
if configuration_variables.SERVER_PROTOCOL == "tcp":
    functions.replace_text_in_file("explicit-exit-notify 1", "explicit-exit-notify 0", server_conf_path)

## (Optional) Point to Non-Default Credentials
functions.replace_text_in_file("cert server.crt", "cert " + configuration_variables.SERVER_NAME + ".crt", server_conf_path)
functions.replace_text_in_file("key server.key", "key " + configuration_variables.SERVER_NAME + ".key", server_conf_path)

## Some of the extra adjustments not indicated in the Digital Ocean tutorial
functions.replace_text_in_file("server 10.8.0.0 255.255.255.0", "server " + configuration_variables.OPENVPN_SUBNET + " " + configuration_variables.OPENVPN_SUBNET_MASK, server_conf_path)
if configuration_variables.ALLOW_CLIENT_TO_CLIENT == "yes":
    functions.replace_text_in_file(";client-to-client", "client-to-client", server_conf_path)
if configuration_variables.ALLOW_DUPLICATE_CN == "yes":
    functions.replace_text_in_file(";duplicate-cn", "duplicate-cn", server_conf_path)

# Step 6 — Adjusting the Server Networking Configuration
functions.replace_text_in_file("#net.ipv4.ip_forward=1", "net.ipv4.ip_forward=1", "/etc/sysctl.conf")
functions.execute_command(["sysctl", "-p"])
shutil.copy2(auxiliary_files_path + "/before.rules", installation_dir + "/")
functions.replace_text_in_file("10.8.0.0/8", configuration_variables.OPENVPN_SUBNET + "/" + configuration_variables.OPENVPN_SUBNET_MASK_IN_BITS, installation_dir + "/before.rules")
interface = functions.get_default_network_device()
functions.replace_text_in_file("eth0", interface, installation_dir + "/before.rules")
before_rules_to_add = functions.read_file(installation_dir + "/before.rules")
if not before_rules_to_add in functions.read_file("/etc/ufw/before.rules"):
    functions.replace_text_in_file("# Don't delete these required lines, otherwise there will be errors", before_rules_to_add + "\n\n# Don't delete these required lines, otherwise there will be errors", "/etc/ufw/before.rules")
functions.replace_text_in_file("DEFAULT_FORWARD_POLICY=\"DROP\"", "DEFAULT_FORWARD_POLICY=\"ACCEPT\"", "/etc/default/ufw")
functions.execute_command(["ufw", configuration_variables.FIREWALL_MODE, configuration_variables.SERVER_PORT + "/" + configuration_variables.SERVER_PROTOCOL])
functions.execute_command(["ufw", configuration_variables.FIREWALL_MODE, "OpenSSH"])
functions.execute_command(["ufw", "disable"])
functions.execute_and_send_input_to_command(["ufw", "enable"], "y\n")

# Step 7 — Starting and Enabling the OpenVPN Service
functions.execute_command(["systemctl", "start", "openvpn@" + configuration_variables.SERVER_NAME])
functions.execute_command(["systemctl", "enable", "openvpn@" + configuration_variables.SERVER_NAME])

# Step 8 — Creating the Client Configuration Infrastructure
functions.makedirs(client_configs_dir + "/files")
...
functions.replace_text_in_file("remote my-server-1 1194\n", "", base_conf_path)
functions.replace_text_in_file(";remote my-server-2 1194\n", remotes_full, base_conf_path)
if configuration_variables.ENABLE_REMOTE_RANDOM == "yes":
    functions.replace_text_in_file(";remote-random", "remote-random", base_conf_path)
functions.replace_text_in_file("proto udp", "proto " + configuration_variables.SERVER_PROTOCOL, base_conf_path)
functions.replace_text_in_file(";user nobody", "user nobody", base_conf_path)
functions.replace_text_in_file(";group nogroup", "group nogroup", base_conf_path)
...
functions.execute_command(["systemctl", "restart", "openvpn@" + configuration_variables.SERVER_NAME])
...
print("")
print("")
functions.execute_command(["systemctl", "status", "openvpn@" + configuration_variables.SERVER_NAME])
